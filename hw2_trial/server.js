...
MongoClient.connect(url, function(err, db) {
    ...
    console.log("資料庫連接成功")
    //將response的資料存在mongodb 資料庫
    db.collection("B05901181").insert(data,function(err,result){
        if(err){
...
app.get('/query', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        db.collection("B05901181").find().toArray(function(err,data){
            console.log("進來查詢");
            //var response_find = {}
            for(var i = 0;i<data.length;i++){
                if(response.data.name==data[i].name){
                  //  response_find = {
                      console.log("get in")
                      response = {
                        result: true,
...
app.post('/update', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        db.collection("B05901181").updateOne(
            filter,
            {$set:update,function(err,result){
                if(err){
                    console.log("更新資料失敗"+err.message)
                    return
            }
            console.log("更新成功")
            console.dir(result)
...
app.post('/delete', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        db.collection("B05901181").deleteOne(filter,function(result){
            console.dir(result)
          })
